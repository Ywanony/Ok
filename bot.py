...
# ==================== CONFIGURATION ====================
BOT_TOKEN = "8619773434:AAELJWNVdXcqxh2cheyXSgx87DpLIcK4aBc"
...
# Helper: VIP Plans Selection Keyboard (₹199 / ₹399 / ₹499)
def get_membership_plans_keyboard(category_code: str):
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="⭐ [ Starter Plan - ₹199 / 30 Days ]", callback_data=f"plan_199_{category_code}")],
            [InlineKeyboardButton(text="⚡ [ Standard Plan - ₹399 / 6 Months ]", callback_data=f"plan_399_{category_code}")],
            [InlineKeyboardButton(text="🔥 [ Lifetime VIP Plan - ₹499 / Lifetime ]", callback_data=f"plan_499_{category_code}")],
            [InlineKeyboardButton(text="« [ Back to Categories ]", callback_data="buy_membership")],
        ]
    )


# Helper: Admin Control Panel Keyboard
...
async def notify_all_admins(bot: Bot, text: str, reply_markup: InlineKeyboardMarkup, photo_id: str = None):
    for admin_id in ADMIN_IDS:
        try:
            if photo_id:
                await bot.send_photo(chat_id=admin_id, photo=photo_id, caption=text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
            else:
                await bot.send_message(chat_id=admin_id, text=text, reply_markup=reply_markup, parse_mode=ParseMode.MARKDOWN)
        except Exception as e:
...
async def receive_screenshot(message: Message, state: FSMContext):
    ...
    safe_fullname = escape_markdown_safe(user.full_name)
