...
async def process_all_collection(callback: CallbackQuery, state: FSMContext):
    ...
    category_photo = CATEGORY_PHOTOS.get("cat_all", CUSTOM_PLAN_QR)
    ...
    await safe_edit_message(callback.message, details_text, keyboard, photo_url=category_photo)
    await callback.answer()
...
async def process_selected_plan(callback: CallbackQuery, state: FSMContext):
    ...
    category_photo = CATEGORY_PHOTOS.get(cat_code, CUSTOM_PLAN_QR)
