...
ADMIN_IDS = [7673557544, 8459158216]  
...
ALL_COLLECTION_PLANS = {
    "299": {"price": 299, "days": 30, "label": "All Collection Starter - ₹299", "duration_text": "30 Days"},
    "599": {"price": 599, "days": 180, "label": "All Collection Mega - ₹599", "duration_text": "6 Months"},
    "999": {"price": 999, "days": 9999, "label": "All Collection Ultimate - ₹999", "duration_text": "Lifetime"}
}
...
# Permanent Fix: Network/DNS Auto-Retry Wrapper for Supabase requests
def supabase_request_with_retry(query_func, retries=3, delay=2):
    for attempt in range(retries):
        try:
            ...
            err_str = str(e)
            if "11001" in err_str or "getaddrinfo" in err_str or "Temporary failure" in err_str:
                logging.warning(f"DNS/Network error encountered (Attempt {attempt + 1}/{retries}): {e}. Retrying in {delay}s...")
...
def log_user_to_db(user):
    try:
        supabase_request_with_retry(lambda: supabase.table("bot_users").upsert({
            "user_id": user.id,
...
def get_membership_plans_keyboard(category_code: str):
    if category_code == "cat_all":
        return InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text=f"⭐ [ {ALL_COLLECTION_PLANS['299']['label']} ]", callback_data=f"plan_299_{category_code}")],
                [InlineKeyboardButton(text=f"⚡ [ {ALL_COLLECTION_PLANS['599']['label']} ]", callback_data=f"plan_599_{category_code}")],
                [InlineKeyboardButton(text=f"🔥 [ {ALL_COLLECTION_PLANS['999']['label']} ]", callback_data=f"plan_999_{category_code}")],
...
async def send_history_view(target_message):
    try:
        res = supabase_request_with_retry(lambda: supabase.table("payments").select("*").order("created_at", desc=True).limit(10).execute())
        records = res.data
        if not records:
...
async def process_selected_plan(callback: CallbackQuery, state: FSMContext):
    ...
    if cat_code == "cat_all":
        plan_info = ALL_COLLECTION_PLANS.get(amount_str, ALL_COLLECTION_PLANS["999"])
    else:
...
async def receive_screenshot(message: Message, state: FSMContext):
    ...
    try:
        supabase_request_with_retry(lambda: supabase.table("payments").insert({
            "user_id": user.id,
            "order_id": order_id,
            "username": username,
            "plan_name": plan_name,
...
async def process_check_status(callback: CallbackQuery):
    ...
    try:
        res = supabase_request_with_retry(lambda: (
...
async def process_custom_link_input(message: Message, state: FSMContext):
    ...
    if not target_user_id or not order_id:
        await message.answer("❌ Error: Session state lost target details. Please try approving again from the notification message.")
        await state.clear()
        return

    try:
        supabase_request_with_retry(lambda: supabase.table("payments").update({"status": "approved", "admin_reason": invite_link}).eq("order_id", order_id).execute())
...
async def process_rejection_reason_input(message: Message, state: FSMContext):
    ...
    if not target_user_id or not order_id:
        await message.answer("❌ Error: Session state lost target details. Please try rejecting again from the notification message.")
        await state.clear()
        return

    try:
        supabase_request_with_retry(lambda: supabase.table("payments").update({"status": "rejected", "admin_reason": reason}).eq("order_id", order_id).execute())
